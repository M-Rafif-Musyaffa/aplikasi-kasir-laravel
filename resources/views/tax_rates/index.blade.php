        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1">
                <div class="bg-white p-6 rounded-lg shadow-md border">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Tambah Opsi Pajak</h3>
                    ...
                    <form action="{{ route('tax-rates.store') }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="rate_percent" id="rate_percent"
                                   ...
                                   value="{{ old('rate_percent') }}" required>
            ...
            <div class="lg:col-span-2"
                ...
                <div class="bg-white p-6 rounded-lg border shadow-md">
                    <h3 class="text-lg font-semibold text-gray-900 mb-4">Daftar Opsi Pajak</h3>
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @forelse ($taxRates as $taxRate)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $taxRate->rate * 100 }}%</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-700">{{ $taxRate->products_count }} Produk</td>
                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                            <a href="{{ route('tax-rates.edit', $taxRate) }}" class="inline-flex items-center px-3 py-2 bg-blue-600 text-white text-sm font-medium rounded-md hover:bg-blue-700">Edit</a>
                                            <button type="button" @click="openDeleteModal('{{ route('tax-rates.destroy', $taxRate) }}')" class="inline-flex items-center px-3 py-2 bg-red-600 text-white text-sm font-medium rounded-md hover:bg-red-700 ml-2">Hapus</button>
                                        </td>
                                    </tr>
                                @empty
                                    <tr><td colspan="4" class="px-6 py-4 text-center text-sm text-gray-500">Belum ada data pajak.</td></tr>
                                @endforelse
                            </tbody>
                        </table>
                    </div>
                ...
                <div x-show="deleteModalOpen" x-transition.opacity class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50" style="display: none;">
                    <div class="relative p-4 w-full max-w-md h-full md:h-auto" @click.away="deleteModalOpen = false">
                        <div class="relative bg-white rounded-lg shadow">
                            <button type="button" @click="deleteModalOpen = false" class="absolute top-3 right-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm p-1.5 ml-auto inline-flex items-center">
                            ...
                            <div class="p-6 text-center">
                                ...
                                <h3 class="mb-5 text-lg font-normal text-gray-500">Yakin ingin menghapus tarif pajak ini?</h3>
                                <form x-bind:action="deleteUrl" method="POST" class="inline-flex">
                                    ...
                                    <button type="button" @click="deleteModalOpen = false" class="text-gray-500 bg-white hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-gray-200 rounded-lg border border-gray-200 text-sm font-medium px-5 py-2.5 hover:text-gray-900 focus:z-10">
                                        Batal
                                    </button>
                                </form>
                            </div>
